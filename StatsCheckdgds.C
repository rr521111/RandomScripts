...
void StatsCheckdgds() {
    ...
    PlotList.push_back(PlotFromMatrix(Vals_table, Form("PREX dg_ds, %d Window", SizeList[isize])));
    ...
    c1->cd(2+(isize*2));
    GausList.push_back(HistFromMatrix(Vals_table, Form("PREX dg_ds, %d Window", SizeList[isize])));
...
vector<vector<Double_t>> OpenRun(TString directory, Int_t runnum, Int_t slugnum, TString ihwps, TString wiens, TString arm, vector<vector<Double_t>> textmatrix, Int_t WindowSize) {
    ...
    TBranch* b4 = mul->GetBranch("asym_bcm_dg_ds");
