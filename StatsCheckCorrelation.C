...
void StatsCheckCorrelation() {
    ...
    PlotList.push_back(PlotFromMatrix(Vals_table, Form("PREX Correlation usl:usr, %d Window", SizeList[isize])));
    ...
    c1->cd(2+(isize*2));
    GausList.push_back(HistFromMatrix(Vals_table, Form("PREX Correlation usl:usr, %d Window", SizeList[isize])));
...
vector<vector<Double_t>> OpenRun(TString directory, Int_t runnum, Int_t slugnum, TString ihwps, TString wiens, TString arm, vector<vector<Double_t>> textmatrix, Int_t WindowSize) {
    ...
    for(int j = 0; j < entries; j++){
        TGraphErrors* uslr = new TGraphErrors();
        Int_t eventcount = 0;
        ...
        for(int k = lastcount; k < b4->GetEntries(); k++){
            ...
            if(errorflag->GetValue(0) == 0){
                uslr->SetPoint(eventcount, values_usl->GetValue(0), values_usr->GetValue(0));
                eventcount++;
        ...
        vals.push_back({runnum+j/static_cast<double>(entries), fit->GetParameter(1), 0, fit->GetParError(1)});
        uslr->Delete();
