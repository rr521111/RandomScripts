...
void ReadingFilesCompareBCMRespins() {
...
vector<vector<Double_t>> OpenRun(TString directory, Int_t runnum, Int_t slugnum, TString ihwps, TString wiens, TString arm, vector<vector<Double_t>> textmatrix) {
    ...
    for(int j = 0; j<entries; j++){
        ...
        mul->Draw(Form("asym_bcm_an_us*1000000000>>bpm4aYhtemp%d()", j), Form("ErrorFlag==0 && Entry$>=%d && Entry$<%d", lastcount, thiscount), "goff");
        bpm4aYhtemp = (TH1F*)gROOT->FindObject(Form("bpm4aYhtemp%d", j));
        
        ofstream outfile;
        outfile.open(outfilePath.Data(),  std::ofstream::out | std::ofstream::app);
